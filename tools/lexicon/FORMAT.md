...
1. Verify header magic and `version == 10`; read metadata.
...
`personTitles`) and `objectPredicativeVerbs`; v9 added `verbClasses`
(Aktionsart), `fragmentParticles`, `focusParticles`, `monthNames`, and merged
the shared-form tenses' person codes (I1s/I3s -> I13s — the ambiguity is now
in the data, not papered over by gates); v10 added `meronymy` (declared
part-whole pairs for bridging reference) and the curated first-name PROPN
entries (gendered feats sharpening anaphora) merged into both lexicons. The binary
